...
```bash
git remote add origin git@github.com:eliavw/missmercs.git
git remote -v
...
```bash
git remote add origin git@github.com:eliavw/missmercs.git; git remote -v; git push origin master
```
...
```bash
conda env create -f dependencies-deploy.yaml -n missmercs
```
...
conda env update -n missmercs -f dependencies-develop.yaml
```
...
```bash
conda activate missmercs
python -m ipykernel install --user --name missmercs --display-name "missmercs"
```
...
1. Go to the [Travis](https://travis-ci.com/eliavw/missmercs) page of this repo.
2. See if it ran.
...
and your site should be online at; [https://eliavw.github.io/missmercs/](https://eliavw.github.io/missmercs/). 
...
> MERCS: An alternative for MissForest when retraining is infeasible, cf. https://eliavw.github.io/missmercs
